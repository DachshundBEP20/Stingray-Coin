...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeStingrayAmount"))
        settings.setValue("nAnonymizeStingrayAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeStingrayAmount = settings.value("nAnonymizeStingrayAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeStingrayAmount"))
        SoftSetArg("-anonymizeStingrayamount", settings.value("nAnonymizeStingrayAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeStingrayAmount:
            nAnonymizeStingrayAmount = value.toInt();
            settings.setValue("nAnonymizeStingrayAmount", nAnonymizeStingrayAmount);
            emit anonymizeStingrayAmountChanged(nAnonymizeStingrayAmount);
            break;
        default:
            break;
        }
