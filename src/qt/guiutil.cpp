...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Stingray"))
        return false;
...
{
    // Convert Stingray:// to Stingray:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Stingray://"))
    {
        uri.replace(0, 12, "Stingray:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Stingray.lnk";
}
...
{
    return GetAutostartDir() / "Stingray.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Stingray-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Stingray-qt [" + tr("command-line options") + "]                     " + "\n";
